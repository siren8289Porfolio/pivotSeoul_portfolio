...
import { ChevronRight, ChevronLeft, MapPin, Wallet, Coffee, CheckCircle2 } from 'lucide-react';
...
const DISTRICT_RENT: Record<string, number> = {
  '강남구': 180, '서초구': 170, '용산구': 160, '마포구': 120, '성동구': 115, '송파구': 130, '노원구': 65, '은평구': 70, '강서구': 80, '관악구': 68,
};

const steps = [
  { label: '거주·직장', icon: MapPin },
  { label: '소득·지출', icon: Wallet },
  { label: '라이프스타일', icon: Coffee },
];
...
export function OnboardingYouth() {
  ...
  const monthlySurplus = profile.monthlyIncome - profile.monthlyHousing - profile.monthlyLiving;
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 w-full">
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        <button onClick={() => step === 0 ? navigate('/stage') : setStep(step - 1)} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: c.card, border: `1px solid ${c.border}`, color: c.textSec }}>
          <ChevronLeft size={16} />
        </button>
        <div className="flex-1">
          <p style={{ color: c.textMuted, fontSize: '0.72rem' }}>청년기 STEP {step + 1} / {steps.length}</p>
          ...
          return (
            <div key={s.label} className={`relative z-10 flex items-start flex-1 min-w-0 ${i === 0 ? 'justify-start' : i === steps.length - 1 ? 'justify-end' : 'justify-center'}`}>
              <div className="relative z-10 flex flex-col items-center shrink-0 w-12">
                <div className="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300" style={{ background: isDone ? c.success : isActive ? c.primary : c.badgeBg, color: isDone || isActive ? 'white' : c.textMuted, boxShadow: isActive ? `0 0 15px ${c.primaryBg}` : 'none' }}>
                  {isDone ? <CheckCircle2 size={18} /> : <Icon size={18} />}
                </div>
                <span className="absolute top-14 whitespace-nowrap text-center" style={{ color: isActive ? c.primary : isDone ? c.success : c.textMuted, fontSize: '0.9rem', fontWeight: 600, lineHeight: 1.2 }}>{s.label}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-none rounded-2xl p-5 md:p-6" style={{ background: c.card, border: `1px solid ${c.cardBorder}`, boxShadow: c.cardShadow }}>
          ...
          <div className="space-y-5">
            <div>
              ...
              {DISTRICT_RENT[profile.currentDistrict] && (
                <p style={{ color: c.primary, fontSize: '0.75rem', marginTop: '6px' }}>📍 {profile.currentDistrict} 평균 월세: {DISTRICT_RENT[profile.currentDistrict]}만원대</p>
              )}
            </div>
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {['월세', '전세', '자가'].map(t => (
                  <button key={t} onClick={() => setHousingType(t)} className="py-2 rounded-xl transition-all" style={{ background: housingType === t ? c.primaryBg : (isDark ? 'rgba(15,23,42,0.4)' : '#F8FAFC'), border: `1px solid ${housingType === t ? c.primary : c.border}`, color: housingType === t ? c.primary : c.textSec, fontSize: '0.88rem' }}>
                    {t}
                  </button>
                ))}
              </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <SliderField label="월 소득 (세후)" value={profile.monthlyIncome} min={150} max={800} step={10} unit="만원" onChange={(v: number) => updateProfile({ monthlyIncome: v })} hint="서울 청년 평균: 280만원" color={c.primary} c={c} />
            ...
            <SliderField label="월 생활비" value={profile.monthlyLiving} min={50} max={350} step={10} unit="만원" onChange={(v: number) => updateProfile({ monthlyLiving: v })} color="#818CF8" c={c} />
            <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: isDark ? 'rgba(15,23,42,0.5)' : '#F8FAFC', border: `1px solid ${monthlySurplus > 50 ? c.successBorder : c.errorBorder}` }}>
              <span style={{ color: c.textSec, fontSize: '0.85rem' }}>월 예상 잔여</span>
              <span style={{ fontSize: '1.1rem', fontWeight: 700, color: monthlySurplus > 50 ? c.success : c.error }}>{monthlySurplus >= 0 ? '+' : ''}{monthlySurplus}만원</span>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <SliderField label="현재 저축액" value={profile.savings} min={0} max={20000} step={500} unit="만원" onChange={(v: number) => updateProfile({ savings: v })} color={c.success} c={c} />
            <div className="p-4 rounded-xl" style={{ background: isDark ? 'rgba(99,102,241,0.08)' : 'rgba(99,102,241,0.05)', border: `1px solid ${c.primaryBorder}` }}>
              <p style={{ color: c.accent, fontSize: '0.8rem', fontWeight: 600, marginBottom: '8px' }}>📋 청년기 입력 요약</p>
              <div className="grid grid-cols-2 gap-y-1.5 gap-x-4 text-xs" style={{ color: c.textSec }}>
                ...
                <span>저축: {profile.savings.toLocaleString()}만원</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between mt-4 shrink-0">
        <button onClick={() => step === 0 ? navigate('/stage') : setStep(step - 1)} className="flex items-center gap-2 px-4 py-2 rounded-xl" style={{ background: c.card, border: `1px solid ${c.border}`, color: c.textSec, fontSize: '0.88rem' }}>
          <ChevronLeft size={15} /> 이전
        </button>
        <button
          onClick={() => step < steps.length - 1 ? setStep(step + 1) : navigate('/scenario')}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl"
          style={{ background: 'linear-gradient(135deg, #6366F1, #818CF8)', color: 'white', fontSize: '0.9rem', fontWeight: 600, boxShadow: '0 0 20px rgba(99,102,241,0.35)' }}
        >
